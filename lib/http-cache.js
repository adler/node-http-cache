...
function attachToRequest(req, res, next) {
	...
	res.end = function(data, encoding) {
		...
		if (!ccontrol || /public/.test(ccontrol) === true || $this.options.forceCache) { // it's OK to cache
			var cache = {
			...
			if (cache.body && cache.body.length > 1024 &&
				...
				/html/.test(cache.headers["content-type"]) === true
				) { // if not already gzipped, and big enough, and is html lets zip it
...
function canCache(req, res, cb) {
	...
	if (req.method !== 'GET' ||
		...
		(ccontrol && /public/.test(ccontrol) === false && ! this.options.forceCache)
		) {
		cb(null, false);
